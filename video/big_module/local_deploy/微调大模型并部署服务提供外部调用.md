...
或者可以直接docker compose部署,因为默认从国外下载的，可能访问不到所以直接指定了国内的（HF_ENDPOINT=https://hf-mirror.com），默认的下载地址（https://huggingface.co），完整命令
```shell
version: "3.9"
...
services:
  llamafactory:
    ...
    ipc: host
    volumes:
      # 覆盖系统自带的 dataset_info.json
      - ./data/dataset_info.json:/app/data/dataset_info.json
      # 本地 datasets 目录，里面放 magic_conch.json 等训练数据
...
语言可以自己选择，这里使用的是`DeepSeek-R1-1.5B-Distill`,模型比较小的蒸馏模型。
